	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
			...
			* a 0 if the strings are equal.
			...
			if ( 0 === strcasecmp( $item->attr_title, 'divider' ) && 1 === $depth ) {
				...
				if( $args->has_children && 0 === $depth ){
					...
					$item_output .= ' <i class="fa fa-chevron-right" aria-hidden="true"></i></a>';
				}elseif( $args->has_children && 0 != $depth && is_rtl() ){
					$item_output .= ' <i class="fa fa-chevron-left" aria-hidden="true"></i></a>';
				}else{
					$item_output .= '</a>';
				}
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				echo '>';
				echo '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="'. esc_html__( 'Add a Menu', 'humane' ) .'">' . esc_html__( 'Add a Menu', 'humane' ) . '</a></li>';
				echo '</ul>';
				if ( $container ) {
					echo '</' . esc_attr( $container ) . '>'; }
			}
